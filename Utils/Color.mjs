class CColor {
	...
	constructor(color) {
		if (!(color instanceof CColor)) {
			if (typeof color === 'string')
				color = CColor.toColorObject(color);
		}
	...
	toName() {
		let r, g, b, colorhexs = CColor.getColorArr('hexs');
		for (i = 0; i < colorhexs.length; i++) {
			r = parseInt(colorhexs[i].substr(0, 2), 16);
			g = parseInt(colorhexs[i].substr(2, 2), 16);
			b = parseInt(colorhexs[i].substr(4, 2), 16);
			if (this.red == r && this.green == g && this.blue == b) {
				return CColor.getColorArr('names')[i];
			}
		}
		return "";
	}
	toHexString() {
		let r = CColor.toHex(this.red);
		let g = CColor.toHex(this.green);
		let b = CColor.toHex(this.blue);
		return "#" + r + g + b;
	}
	toRgb() {
	...
	saturate(n) {
		...
		rgb = CColor.hslToRgb(this.hue, this.sat, this.lightness);
		color = CColor.colorObject(rgb, this.opacity, this.hue, this.sat);
		return color;
	}
	desaturate(n) {
		let x, rgb, color;
	...
	static toColorObject(c) {
		...
		if (c.substr(0, 3) == "rgb" || c.substr(0, 3) == "hsl" || c.substr(0, 3) == "hwb" || c.substr(0, 4) == "ncol" || c.substr(0, 4) == "cmyk") {
			...
			if (typ == "rgb") {
				...
				for (i = 0; i < arrlength; i++) {
					...
					if (isNaN(arr[i])) { return CColor.emptyObject(); }
			...
			if (typ == "hsl" || typ == "hwb" || typ == "ncol") {
				...
				if (typ == "hsl") { rgb = CColor.hslToRgb(arr[0], arr[1], arr[2]); hue = Number(arr[0]); sat = Number(arr[1]); }
				if (typ == "hwb") { rgb = CColor.hwbToRgb(arr[0], arr[1], arr[2]); }
				if (typ == "ncol") { rgb = CColor.ncolToRgb(arr[0], arr[1], arr[2]); }
				if (opacity == true) { a = Number(arr[3]); }
			}
			if (typ == "cmyk") {
				while (arr.length < arrlength) { arr.push("0"); }
				...
				rgb = CColor.cmykToRgb(arr[0], arr[1], arr[2], arr[3]);
				if (opacity == true) { a = Number(arr[4]); }
			}
		} else if (c.substr(0, 3) == "ncs") {
			rgb = CColor.ncsToRgb(c);
		} else {
			match = false;
			colornames = CColor.getColorArr('names');
			for (i = 0; i < colornames.length; i++) {
				if (c.toLowerCase() == colornames[i].toLowerCase()) {
					colorhexs = CColor.getColorArr('hexs');
					match = true;
					rgb = {
						r: parseInt(colorhexs[i].substr(0, 2), 16),
						g: parseInt(colorhexs[i].substr(2, 2), 16),
			...
			if (match == false) {
				...
				for (i = 0; i < c.length; i++) {
					if (!CColor.isHex(c.substr(i, 1))) { return CColor.emptyObject(); }
				}
		...
		return CColor.colorObject(rgb, a, hue, sat);
	}
	static colorObject(rgb, a, h, s) {
		let hsl, hwb, cmyk, ncol, color, hue, sat;
		if (!rgb) { return CColor.emptyObject(); }
		if (a === null) { a = 1; }
		hsl = CColor.rgbToHsl(rgb.r, rgb.g, rgb.b);
		hwb = CColor.rgbToHwb(rgb.r, rgb.g, rgb.b);
		cmyk = CColor.rgbToCmyk(rgb.r, rgb.g, rgb.b);
		hue = (h || hsl.h);
		sat = (s || hsl.s);
		ncol = CColor.hueToNcol(hue);
		color = {
			red: rgb.r,
			green: rgb.g,
			blue: rgb.b,
		...
		color = CColor.roundDecimals(color);
	...
	static hslToRgb(hue, sat, light) {
		...
		t1 = light * 2 - t2;
		r = CColor.hueToRgb(t1, t2, hue + 2) * 255;
		g = CColor.hueToRgb(t1, t2, hue) * 255;
		b = CColor.hueToRgb(t1, t2, hue - 2) * 255;
		return { r: r, g: g, b: b };
	}
	...
	static hwbToRgb(hue, white, black) {
		...
		rgb = CColor.hslToRgb(hue, 1, 0.50);
		rgbArr[0] = rgb.r / 255;
		rgbArr[1] = rgb.g / 255;
		rgbArr[2] = rgb.b / 255;
		tot = white + black;
	...
	static ncolToRgb(ncol, white, black) {
		...
		return CColor.hwbToRgb(h, white, black);
	}
	...
	static ncsToRgb(ncs) {
		let black, chroma, bc, percent, black1, chroma1, 
			red1, factor1, blue1, 
			red2, green2, blue2, 
			max, factor2, grey, 
			r, g, b;
		ncs = CColor.trim(ncs).toUpperCase();
		ncs = ncs.replace("(", "");
		ncs = ncs.replace(")", "");
